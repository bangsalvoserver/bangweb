...
function getCardFameTokens(card: Card): [string | null, number] {
...
export default function CardView({ cardRef, card, showBackface }: CardProps) {
    ...
    const [tokenSprite, numFame] = getCardFameTokens(card);
    ...
    return (
        <div ref={divRef} style={style} className={classes.join(' ')}
            ...
            { cardImage ? <div className="card-front">
                ...
                {card.tokens.cube > 0 && <div className="card-tokens">
                ...
                {tokenSprite !== null && numFame > 0 && <div className="card-tokens">
                    {[...Array(numFame)].map((item, i) => (
                        <img key={i} className="card-fame" src={tokenSprite} alt="" />
                    ))}
                </div>}
