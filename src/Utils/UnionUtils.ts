...
export function createUnionFunction<Context, Update extends object>(functions: {
    [K in Update as keyof K]?: (this: Context, value: K[keyof K]) => void;
}) {
    return (context: Context, update: Update) => {
        const [updateType, updateValue] = Object.entries(update)[0];
        if (updateType in functions) {
            const fn = functions[updateType as keyof typeof functions];
            (fn as (this: Context, update: unknown) => void).call(context, updateValue);
        }
    };
}
