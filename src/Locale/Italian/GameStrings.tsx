...
export const GAME_STRINGS: GameStringRegistry = {
    ...
    PROMPT_TARGET_IMMUNE: (card, player) =>                                 <>{player} è immune a {card}. Continuare?</>,
    ...
    PROMPT_SINGLE_TARGET: (card) =>                                         <>Stai giocando {card} contro un solo giocatore. Continuare?</>,
    ...
    PROMPT_MISS_SUSANNA: (card, count, max_count) =>                        <>Hai giocato solo {count === 1 ? "una carta" : `${count} carte`} su {max_count}. Prenderai un danno per {card}. Continuare?</>,
    PROMPT_SUICIDE: (card) =>                                               <>Giocare {card} porta al suicidio. Continuare?</>,
    PROMPT_DEATH: () =>                                                     <>Puoi giocare una birra per salvarti. Continuare?</>,
    PROMPT_CLAIM_NO_TARGET: () =>                                           <>Non ci sono leggende da colpire. Continuare?</>,
    ...
    PROMPT_TRACK_SELF: (card) =>                                            <>Non c'è motivo di tracciarti da solo con {card}. Continuare?</>,
    ...
    STATUS_CHOOSE_SCENARIO: () =>                                           <>Scegli la carta scenario finale</>,
    STATUS_CHOOSE_SCENARIO_OTHER: (player) =>                               <>{player} deve scegliere la carta scenario finale</>,
    STATUS_PREDRAW: () =>                                                   <>Scegli per quale carta estrarre</>,
    STATUS_PREDRAW_OTHER: (player) =>                                       <>{player} deve scegliere per quale carta estrarre</>,
    STATUS_PREDRAW_FOR: (card) =>                                           <>Estrai per {card}</>,
    STATUS_PREDRAW_FOR_OTHER: (player, card) =>                             <>{player} deve estrarre per {card}</>,
    ...
    STATUS_CHECK_FOR: (card, target) =>                                     <>Scegli quale carta fare estrarre a {target} per {card}</>,
    STATUS_CHECK_FOR_OTHER: (player, card, target) =>                       <>{player} deve scegliere quale carta fare estrarre a {target} per {card}</>,
    ...
    STATUS_TORNADO2: (card, target) =>                                      <>{card} ... Scegli due carte da dare a {target}</>,
    STATUS_TORNADO2_OTHER: (player, card, target) =>                        <>{card} ... {player} deve scegliere due carte da dare a {target}</>,
    ...
    STATUS_ROLES_SHUFFLED: () =>                                            <>I ruoli vengono mescolati!</>,
    ...
    STATUS_FRANKIE_CANTON: (card, target_card) =>                           <>{card} ti ruba un cubo da {target_card}</>,
    STATUS_FRANKIE_CANTON_OTHER: (player, card, target_card) =>             <>{card} ruba un cubo da {target_card} di {player}</>,
    ...
    STATUS_ANNIE_OAKEY: (card) =>                                           <>{card} ... Puoi provare a indovinare il seme o il colore della prossima carta pescata</>,
    STATUS_ANNIE_OAKEY_OTHER: (player, card) =>                             <>{card} ... {player} può provare a indovinare il seme o il colore della prossima carta pescata</>,
    STATUS_LAURA_BILLION: (card, target_card) =>                            <>{card} ... puoi scartare una carta per pescare {target_card}.</>,
    STATUS_LAURA_BILLION_OTHER: (player, card, target_card) =>              <>{card} ... {player} può scartare una carta per pescare {target_card}.</>,
    ...
    STATUS_KIT_CARLSON_LEGEND: (card) =>                                    <>{card} ... puoi dare una delle carte pescate a un altro giocatore</>,
    ...
    STATUS_DISCARD_FEAT: () =>                                              <>Scarta un'impresa</>,
    STATUS_DISCARD_FEAT_OTHER: (player) =>                                  <>{player} deve scartare un'impresa</>,
    STATUS_PERFORM_FEAT: (card) =>                                          <>Puoi compiere l'impresa {card}</>,
    STATUS_PERFORM_FEAT_OTHER: (card, player) =>                            <>{player} può compiere l'impresa {card}</>,
    STATUS_PERFORM_A_FEAT: (card) =>                                        <>Puoi compiere un'impresa</>,
    STATUS_PERFORM_A_FEAT_OTHER: (player) =>                                <>{player} può compiere un'impresa</>,
    STATUS_CLAIM_FEAT: (card) =>                                            <>Puoi rivendicare l'impresa {card}</>,
    STATUS_CLAIM_FEAT_OTHER: (card, player) =>                              <>{player} può rivendicare l'impresa {card}</>,
    STATUS_CLAIM_A_FEAT: (card) =>                                          <>Puoi rivendicare un'impresa</>,
    STATUS_CLAIM_A_FEAT_OTHER: (player) =>                                  <>{player} può rivendicare un'impresa</>,
    STATUS_BOAST_ADD_FAME: () =>                                            <>Scegli un'impresa su cui mettere un segnalino fama</>,
    STATUS_BOAST_ADD_FAME_OTHER: (player) =>                                <>{player} deve scegliere un'impresa su cui mettere un segnalino fama</>,
    STATUS_BOAST_FEAT: () =>                                                <>Puoi vantare un'impresa</>,
    STATUS_BOAST_FEAT_OTHER: (player) =>                                    <>{player} può vantare un'impresa</>,
    STATUS_DAMAGE_LEGEND: () =>                                             <>Puoi far perdere un punto vita a una leggenda</>,
    STATUS_DAMAGE_LEGEND_OTHER: (player) =>                                 <>{player} può far perdere un punto vita a una leggenda.</>,
    STATUS_CAN_DRAW: (card, ncards) =>                                      <>{card} ... puoi pescare {ncards === 1 ? 'una carta' : `${ncards} carte`}.</>,
    STATUS_CAN_DRAW_OTHER: (card, player, ncards) =>                        <>{card} ... {player} può pescare {ncards === 1 ? 'una carta' : `${ncards} carte`}.</>,
    STATUS_BALLAD_STEAL: (card, player) =>                                  <>{card} ... puoi rubare una carta dalla mano di {player}.</>,
    ...
    STATUS_FALCON_OTHER: (card, player, target) =>                          <>{card} ... {target} sta guardando la mano di {player}</>,
    STATUS_FALCON_TARGET: (card, player) =>                                 <>{card} ... {player} vuole guardare la tua mano.</>,
    STATUS_FALCON_TARGET_OTHER: (target, card, player) =>                   <>{card} ... {player} vuole guardare la mano di {target}.</>,
    STATUS_GRIZZLY: (card) =>                                               <>{card} ti attacca!</>,
    STATUS_GRIZZLY_OTHER: (player, card) =>                                 <>{card} contro {player}</>,
    STATUS_STAMPEDE: (card) =>                                              <>{card} ... devi scartare tutte le carte di fronte a te.</>,
    STATUS_STAMPEDE_OTHER: (target, card) =>                                <>{card} ... {target} deve scartare tutte le carte di fronte a sé.</>,
    STATUS_TRACK_PLAYER: (card) =>                                          <>Scegli chi tracciare per {card}.</>,
    ...
    ERROR_TARGET_NOT_PLAYER: () =>                                          <>Il bersaglio non è un giocatore</>,
    ...
    ERROR_CANT_HEAL_PAST_FULL_HP: () =>                                     <>Non puoi curarti oltre il massimo dei tuoi punto vita</>,
    ...
    ERROR_MANDATORY_CARD: (card) =>                                         <>Sei obbligato a giocare {card}</>,
    ...
    LOG_GAME_START: () =>                                                   <><hr/>La partita è iniziata</>,
    ...
    LOG_WINNER_SHERIFF: () =>                                               <><hr/>Lo Sceriffo e i Vice vincono la partita!</>,
    LOG_WINNER_OUTLAW: () =>                                                <><hr/>I Fuorilegge vincono la partita!</>,
    LOG_WINNER_RENEGADE: () =>                                              <><hr/>Il Rinnegato vince la partita!</>,
    ...
    LOG_FEATS_RESHUFFLED: () =>                                             <>Il mazzo delle imprese viene rimescolato</>,
    LOG_TURN_START: (player) =>                                             <><hr/>È il turno di {player}</>,
    ...
    LOG_DISCARDED_FEAT: (player, card) =>                                   <>{player} scarta l'impresa {card}</>,
    ...
    LOG_REVEALED_ANY_CARD: (card) =>                                        <>{card} è rivelata</>,
    LOG_CHECK_DREW_CARD: (card, player, target_card) =>                     <>{player} estrae {target_card} per {card}</>,
    LOG_CHECK_CARD_DRAWN: (card, drawn_card) =>                             <>{drawn_card} è estratta per {card}</>,
    ...
    LOG_BECOME_LEGEND: (player, card) =>                                    <>{player} diventa una Leggenda del West con {card}</>,
    LOG_DAMAGED_LEGEND: (player, target) =>                                 <>{player} fa perdere un punto vita alla leggenda {target}</>,
    LOG_FEAT_PERFORMED: (player, card) =>                                   <>{player} compie l'impresa {card}</>,
    LOG_FEAT_CLAIMED: (player, card) =>                                     <>{player} rivendica l'impresa {card}</>,
    LOG_FEAT_BOASTED: (player, card) =>                                     <>{player} vanta l'impresa {card}</>,
    LOG_TRACKED_PLAYER: (player, card, target) =>                           <>{player} traccia {target} per {card}</>,
    LOG_MOVE_TRAP_ON: (card, player, target) =>                             <>{player} sposta {card} su {target}</>,
